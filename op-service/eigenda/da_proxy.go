...
func (c *EigenDAClient) RetrieveBlob(ctx context.Context, requestID []byte) ([]byte, error) {
	c.log.Info("Attempting to retrieve blob from EigenDA with request id", "requestID", requestID)
	reply, err := c.GetBlobStatus(ctx, requestID)
	if err != nil {
		return nil, err
	}
	commitment, err := EncodeCommitment(reply.Info)
	if err != nil {
		return nil, err
	}
