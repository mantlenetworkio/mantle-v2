...
import (
	...
	binding "github.com/ethereum-optimism/optimism/op-service/eigenda/bindings/EigenDAServiceManager"
...
func (v *Verifier) VerifyCert(cert *Certificate) error {
	...
	root := cert.Proof().GetBatchMetadata().GetBatchHeader().GetBatchRoot()
	recordHash := cert.Proof().BatchMetadata.GetSignatoryRecordHash()
	if len(root) > 32 {
		root = root[:32]
	}
	if len(recordHash) > 32 {
		recordHash = recordHash[:32]
	}
	copy(blobHeadersRoot[:], root)
	copy(signatoryRecordHash[:], recordHash)
	...
	err := v.cv.VerifyBatch(&header, cert.Proof().GetBatchId(), signatoryRecordHash, cert.Proof().BatchMetadata.GetConfirmationBlockNumber())
	if err != nil {
		return err
	}
...
func (v *Verifier) VerifyCommitment(expectedCommit *common.G1Commitment, blob []byte) error {
	...
	expectedX.SetBytes(expectedCommit.X)
	expectedY := &fp.Element{}
	expectedY.SetBytes(expectedCommit.Y)
