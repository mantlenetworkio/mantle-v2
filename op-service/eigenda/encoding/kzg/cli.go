...
func CLIFlags(envPrefix string) []cli.Flag {
	return []cli.Flag{
		cli.StringFlag{
			...
			Required: true,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "G1_PATH"),
		},
		cli.StringFlag{
			Name:     G2PathFlagName,
			...
			Required: false,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "G2_PATH"),
		},
		cli.StringFlag{
			Name:     CachePathFlagName,
			Usage:    "Path to SRS Table directory",
			Required: true,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "CACHE_PATH"),
		},
		cli.Uint64Flag{
			Name:     SRSOrderFlagName,
			Usage:    "Order of the SRS",
			Required: true,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "SRS_ORDER"),
		},
		cli.Uint64Flag{
			Name:     SRSLoadingNumberFlagName,
			Usage:    "Number of SRS points to load into memory",
			Required: true,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "SRS_LOAD"),
		},
		cli.Uint64Flag{
			Name:     NumWorkerFlagName,
			Usage:    "Number of workers for multithreading",
			Required: false,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "NUM_WORKERS"),
		...
		cli.BoolFlag{
			...
			Required: false,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "VERBOSE"),
		},
		cli.BoolFlag{
			Name:     CacheEncodedBlobsFlagName,
			Usage:    "Enable to cache encoded results",
			Required: false,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "CACHE_ENCODED_BLOBS"),
		},
		cli.BoolFlag{
			Name:     PreloadEncoderFlagName,
			Usage:    "Set to enable Encoder PreLoading",
			Required: false,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "PRELOAD_ENCODER"),
		},
		cli.StringFlag{
			...
			Required: false,
			EnvVar:   opservice.PrefixEnvVar(envPrefix, "G2_POWER_OF_2_PATH"),
		},
	}
}
