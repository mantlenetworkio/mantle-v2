...
func WithAlloc(alloc types.GenesisAlloc) SimulatedBackendOpt {
	return func(s *simulatedBackendConfig) {
...
func NewSimulatedBackendWithOpts(opts ...SimulatedBackendOpt) *SimulatedBackend {
	config := &simulatedBackendConfig{
		genesis:   core.Genesis{Config: params.AllEthashProtocolChanges, GasLimit: 100000000, Alloc: make(types.GenesisAlloc)},
	...
	if !b.blockchain.Config().IsLondon(head.Number) {
		...
		if call.GasPrice != nil {
			...
			if call.GasFeeCap.BitLen() > 0 || call.GasTipCap.BitLen() > 0 {
				call.GasPrice = BigMin(new(big.Int).Add(call.GasTipCap, head.BaseFee), call.GasFeeCap)
			}
		}
	}
...
func (fb *filterBackend) CurrentHeader() *types.Header {
	hdr, _ := fb.HeaderByNumber(context.TODO(), rpc.LatestBlockNumber)
	return hdr
}
