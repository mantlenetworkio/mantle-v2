...
func RecoverAddressFromSign(sign, msg []byte) (common.Address, error) {
	if len(sign) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("wrong size for signature: got %d, want %d", len(sign), crypto.SignatureLength)
	}

	publicKey, err := crypto.SigToPub(msg, sign)
	if err != nil {
