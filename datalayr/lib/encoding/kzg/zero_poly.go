...
type ZeroPolyFn func(missingIndices []uint64, length uint64) ([]bls.Fr, []bls.Fr, error)

func (fs *FFTSettings) makeZeroPolyMulLeaf(dst []bls.Fr, indices []uint64, domainStride uint64) error {
...
func (fs *FFTSettings) reduceLeaves(scratch []bls.Fr, dst []bls.Fr, ps [][]bls.Fr) ([]bls.Fr, error) {
	n := uint64(len(dst))
...
func (fs *FFTSettings) ZeroPolyViaMultiplication(missingIndices []uint64, length uint64) ([]bls.Fr, []bls.Fr, error) {
	if len(missingIndices) == 0 {
		return make([]bls.Fr, length, length), make([]bls.Fr, length, length), nil
	}
