...
func (p *SRSTable) Precompute(dim, dimE, l, m uint64, filePath string, numWorker uint64) [][]bls.G1Point {
	...
	fftPoints := make([][]bls.G1Point, l, l)
...
func (p *SRSTable) precomputeWorker(fs *kzg.FFTSettings, m, dim, dimE uint64, jobChan <-chan uint64, l uint64, results chan DispatchReturn) error {
	for j := range jobChan {
...
func (p *SRSTable) readWorker(
	...
	for b := range jobChan {
		slicePoints := make([]bls.G1Point, dimE*2, dimE*2)
