 ...
 * 3) The data that the operator was storing for the BOMB datastore, when hashed with the operator's address and the DETONATION datastore's
...
contract DataLayrBombVerifier is Initializable {
    ...
    /// @notice struct used to resolve stack-too-deep errors in the `verifyBomb` function
    struct VerificationData {
        uint256 operatorActiveFromBlockNumber;
     ...
     * abstract away some common logic.
     */
    constructor(
     ...
     * Ultimately, the BOMB condition is checked against a combined hash of the data stored in the BOMB datastore, the operator's address, and
     * the header hash of the DETONATION datastore
     * 
     * @dev DataLayr will make proof of custody keys private in a future upgrade.
     *
    ...
    function verifyBomb(
        ...
        */
        ...
        {
            // check that the operator was active at the `verificationData.operatorActiveFromBlockNumber`
            require(dlRegistry.checkOperatorActiveAtBlockNumber(operator, verificationData.operatorActiveFromBlockNumber, verificationData.stakeHistoryIndex),
                "DataLayrBombVerifier.verifyBomb: operator was not active at verificationData.operatorActiveFromBlockNumber");
            ...
            for (uint256 i = 0; i < ultimateBombDataStoreIndex; ++i) {
                ...
                if (dataStoreProofs.bombDataStores[i].metadata.referenceBlockNumber >= verificationData.operatorActiveFromBlockNumber) {
                    ...
                    if (dataStoreProofs.bombDataStores[i].metadata.signatoryRecordHash != bytes32(0)) {
                        require(dataStoreProofs.bombDataStores[i].metadata.signatoryRecordHash
            ...
            require(
                dataStoreProofs.bombDataStores[ultimateBombDataStoreIndex].metadata.referenceBlockNumber
                    >= verificationData.operatorActiveFromBlockNumber,
            ...
            // require that the detonation is happening for a datastore using the operators stake
            require(
                dataStoreProofs.detonationDataStore.metadata.referenceBlockNumber >= verificationData.operatorActiveFromBlockNumber,
        ...
        require(
            uint256(
                keccak256(
                    abi.encodePacked(disclosureProof.poly, operator, dataStoreProofs.detonationDataStore.metadata.headerHash)
                )
            ) < BOMB_THRESHOLD,
