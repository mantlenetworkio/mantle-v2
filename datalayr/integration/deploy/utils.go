...
const (
	useDocker    = false
	foundryImage = "ghcr.io/foundry-rs/foundry:nightly-8c4294c1d2321e20a3543fbd9a813d47053a8303"
)
...
func removeFile(name string) {
	err := os.Remove(name)
	if err != nil {
		log.Panicf("Failed to remove file. Err: %s", err)
	}
}
...
func copyDirectory(source, dest string) {
	cmd := exec.Command("cp", "--recursive", source, dest)
	cmd.Run()
}

func removeDirectory(path string) {
	cmd := exec.Command("rm", "--recursive", "--force", path)
	cmd.Run()
}

// Executes arbitrary bash.
...
func execYarnCmd(command string) {
	...
	if err != nil {
		log.Print(fmt.Sprint(err) + ": " + stderr.String())
		log.Panicf("Failed to execute yarn command. Err: %s", err)
	} else {
		log.Print(out.String())
	}
...
func execForgeScript(script, privateKey string, deployer ContractDeployer) {
