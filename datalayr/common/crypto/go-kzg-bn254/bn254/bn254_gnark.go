...
func MulG1(dst *G1Point, a *G1Point, b *Fr) {
	...
	var t big.Int
	(*fr.Element)(b).ToBigIntRegular(&t)
...
// //Commits to a higher the g1 points that are `shift` higher indexes in `numbers`
// func ShiftedLinCombG1(numbers []G1Point, factors []Fr, shift int) *G1Point {
// 	fmt.Println(len(numbers), len(factors), shift)
// 	out := new(G1Point)
// 	CopyG1(out, &ZeroG1)
// 	out1 := *(*bn.G1Affine)(out)
...
// 	for i := 0; i < len(np); i++ {
// 		if i+shift < len(numbers) {
// 			np[i] = *(*bn.G1Affine)(&numbers[i+shift])
// 		}
// 	}
// 	for i := 0; i < len(fs); i++ {
// 		fs[i] = *(*fr.Element)(&factors[i])
...
func LinCombG1_(numbers []G1Point, factors []Fr) *G1Point {
	out := new(G1Point)
	CopyG1(out, &ZeroG1)
	var tmp, tmp2 G1Point
...
func LinCombG2_(numbers []G2Point, factors []Fr) *G2Point {
	out := new(G2Point)
	CopyG2(out, &ZeroG2)
	var tmp, tmp2 G2Point
	for i := 0; i < len(numbers); i++ {
		MulG2(&tmp, &numbers[i], &factors[i])
		AddG2(&tmp2, out, &tmp)
		CopyG2(out, &tmp2)
	}
	return (*G2Point)(out)
