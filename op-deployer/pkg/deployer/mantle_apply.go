...
func MantleApplyPipeline(
	...
	mantleScripts, err := opcm.NewMantleScripts(l1Host)
	if err != nil {
		return fmt.Errorf("failed to load Mantle scripts: %w", err)
	}
	...
	for _, chain := range intent.Chains {
		...
		pline = append(pline, pipelineStage{
			fmt.Sprintf("deploy-proxies-%s", chainID.Hex()),
			func() error {
				return pipeline.DeployMantleProxies(pEnv, intent, st, chainID)
			},
		}, pipelineStage{
			fmt.Sprintf("deploy-implementations-%s", chainID.Hex()),
			func() error {
				return pipeline.DeployMantleImplementations(pEnv, intent, st, chainID)
			},
		}, pipelineStage{
			fmt.Sprintf("deploy-opchain-%s", chainID.Hex()),
			func() error {
				return pipeline.DeployMantleOPChain(pEnv, intent, st, chainID)
			},
		}, pipelineStage{
			fmt.Sprintf("generate-l2-genesis-%s", chainID.Hex()),
			func() error {
				return pipeline.GenerateMantleL2Genesis(pEnv, intent, bundle, st, chainID)
			},
		})
	}
