...
func ReorgOrphanBlock(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	arsiaTimeOffset := hexutil.Uint64(0)
	sd, _, miner, sequencer, _, verifier, verifierEng, batcher := actionsHelpers.SetupMantleReorgTest(t, actionsHelpers.DefaultRollupTestParams(), &arsiaTimeOffset, isSpanBatch)
...
func ReorgFlipFlop(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	arsiaTimeOffset := hexutil.Uint64(0)
	sd, _, miner, sequencer, _, verifier, verifierEng, batcher := actionsHelpers.SetupMantleReorgTest(t, actionsHelpers.DefaultRollupTestParams(), &arsiaTimeOffset, isSpanBatch)
...
// - Unsafe head is 62
// - Safe head is 42
func DeepReorg(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)

	arsiaTimeOffset := hexutil.Uint64(0)
	// Create actor and verification engine client
	sd, dp, miner, sequencer, seqEngine, verifier, verifierEng, batcher := actionsHelpers.SetupMantleReorgTest(t, &e2eutils.TestParams{
		MaxSequencerDrift:   40,
		SequencerWindowSize: 20,
		ChannelTimeout:      120,
		L1BlockTime:         4,
		AllocType:           config.DefaultAllocType,
	}, &arsiaTimeOffset, isSpanBatch)
	minerCl := miner.L1Client(t, sd.RollupCfg)
...
func RestartOpGeth(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
...
func ConflictingL2Blocks(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	dp := e2eutils.MakeMantleDeployParams(t, actionsHelpers.DefaultRollupTestParams())
	arsiaTimeOffset := hexutil.Uint64(0)
	upgradesHelpers.ApplyArsiaTimeOffset(dp, &arsiaTimeOffset)
	...
	sd, _, miner, sequencer, seqEng, verifier, _, batcher := actionsHelpers.SetupMantleReorgTestActors(t, dp, sd, log, isSpanBatch)
	...
	altBatcher := actionsHelpers.NewL2Batcher(log, sd.RollupCfg, &actionsHelpers.BatcherCfg{
		MinL1TxSize:          0,
		MaxL1TxSize:          128_000,
		BatcherKey:           dp.Secrets.Batcher,
		ForceSubmitSpanBatch: true,
...
func SyncAfterReorg(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	...
	arsiaTimeOffset := hexutil.Uint64(0)
	sd, dp, miner, sequencer, seqEngine, verifier, _, batcher := actionsHelpers.SetupMantleReorgTest(t, &testingParams, &arsiaTimeOffset, isSpanBatch)
	l2Client := seqEngine.EthClient()
	log := testlog.Logger(t, log.LevelDebug)
	addresses := e2eutils.CollectAddresses(sd, dp)
	l2UserEnv := &actionsHelpers.BasicUserEnv[*actionsHelpers.L2Bindings]{
