...
func NormalBatcher(gt *testing.T, isSpanBatch bool) {
...
func L2Finalization(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
...
func L2FinalizationWithSparseL1(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	dp := e2eutils.MakeMantleDeployParams(t, actionsHelpers.DefaultRollupTestParams())
	// Enable MNT as gas token (zero address represents MNT in Mantle)
	dp.DeployConfig.UseCustomGasToken = true
...
func GarbageBatch(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
...
func ExtendedTimeWithoutL1Batches(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	p := &e2eutils.TestParams{
		MaxSequencerDrift:   20, // larger than L1 block time we simulate in this test (12)
		SequencerWindowSize: 24,
...
func BigL2Txs(gt *testing.T, isSpanBatch bool) {
	t := actionsHelpers.NewDefaultTesting(gt)
	p := &e2eutils.TestParams{
	...
	// In Arsia fork, DA footprint is stored in blobGasUsed and limited by gas limit.
	// For high-performance testing with large transactions (120KB each), we need a larger
	// gas limit to accommodate the DA footprint. Single 120KB tx has DA footprint of
	// ~48M (120000 * 400 scalar), so we set gas limit to 500M to allow ~10 large txs per block.
	dp.DeployConfig.L2GenesisBlockGasLimit = 500_000_000
	sd := e2eutils.SetupMantleNormal(t, dp, actionsHelpers.DefaultAlloc)
	...
	for i := 0; i < 40; i++ {
		...
		// Track cumulative DA footprint for Arsia fork
		var cumulativeDAFootprint uint64
		currentBlock := engine.L2Chain().CurrentBlock()
		isArsia := sd.L2Cfg.Config.IsMantleArsia(currentBlock.Time)
		blockGasLimit := currentBlock.GasLimit

		// DA footprint gas scalar (default value, should match L1 attributes)
		const daFootprintGasScalar = uint64(400)
		...
		for n := aliceNonce; ; n++ {
			...
			// Create the transaction first to calculate DA footprint
			tx := types.MustSignNewTx(dp.Secrets.Alice, signer, &types.DynamicFeeTx{
				ChainID:   sd.L2Cfg.Config.ChainID,
				Nonce:     n,
				GasTipCap: big.NewInt(2 * params.GWei),
			...
			// In Arsia fork, check DA footprint limit in addition to gas limit
			if isArsia {
				// Calculate DA footprint for this transaction
				txDAFootprint := tx.RollupCostData().EstimatedDASize().Uint64() * daFootprintGasScalar

				// Check if adding this tx would exceed the block gas limit (DA footprint limit)
				if cumulativeDAFootprint+txDAFootprint > blockGasLimit {
					log.Debug("DA footprint limit reached",
						"cumulative", cumulativeDAFootprint,
						"tx_footprint", txDAFootprint,
						"limit", blockGasLimit)
					break
				}
				cumulativeDAFootprint += txDAFootprint
			}
