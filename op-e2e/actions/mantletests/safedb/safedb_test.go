...
import (
	...
	"github.com/ethereum-optimism/optimism/op-e2e/actions/mantletests/safedb/helpers"
...
func TestRecordSafeHeadUpdates(t *testing.T) {
	tests := []struct {
		name string
		f    func(gt *testing.T, forksname rollup.MantleForkName)
	}{
		{"RecordSafeHeadUpdates", RecordSafeHeadUpdates},
	}
...
func RecordSafeHeadUpdates(gt *testing.T, forksname rollup.MantleForkName) {
	t := actionsHelpers.NewDefaultTesting(gt)
	sd, miner, sequencer, verifier, verifierEng, batcher := helpers.SetupMantleSafeDBTest(t, actionsHelpers.DefaultRollupTestParams(), forksname)
