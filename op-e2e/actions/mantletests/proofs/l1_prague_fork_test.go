...
func Test_ProgramAction_PragueForkAfterGenesis(gt *testing.T) {
	...
	runL1PragueTest := func(gt *testing.T, testCfg *helpers.TestCfg[testCase]) {
		...
		env := helpers.NewL2ProofEnv(t, testCfg, helpers.NewTestParams(),
			...
			func(dp *genesis.DeployConfig) {
				...
				dp.L1OsakaTimeOffset = ptr(hexutil.Uint64(24))  // Activate at second l1 block
			},
		)
	...
	defer matrix.Run(gt)
	matrix.
		AddDefaultTestCasesWithName(dynamiceFeeCase.name, dynamiceFeeCase, helpers.NewForkMatrix(helpers.MantleLimb, helpers.MantleArsia, helpers.MantleLatestFork), runL1PragueTest).
		AddDefaultTestCasesWithName(setCodeCase.name, setCodeCase, helpers.NewForkMatrix(helpers.MantleLimb, helpers.MantleLatestFork), runL1PragueTest)
}
