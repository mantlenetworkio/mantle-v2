...
func setDAFootprintGasScalarViaSystemConfig(t actionsHelpers.Testing, env *helpers.L2ProofEnv, scalar uint16) {
...
func requireL1BlockDAFootprintGasScalarEquals(t actionsHelpers.Testing, env *helpers.L2ProofEnv, scalar uint16) {
	t.Helper()
...
func Test_ProgramAction_JovianDAFootprint(gt *testing.T) {
	...
	runStep := func(t actionsHelpers.Testing, env *helpers.L2ProofEnv, setScalar *uint16) {
	...
	run := func(gt *testing.T, testCfg *helpers.TestCfg[helpers.DeployConfigOverride]) {
		...
		env := helpers.NewL2ProofEnv(t, testCfg, helpers.NewTestParams(), helpers.NewBatcherCfg(), testCfg.Custom)

		arsiaAtGenesis := env.Sequencer.RollupCfg.IsMantleArsia(env.Sequencer.RollupCfg.Genesis.L2Time)
		if !arsiaAtGenesis {
			env.Sequencer.ActBuildL2ToMantleFork(t, forks.MantleArsia)
		}
	...
	tests := map[string]struct {
		...
		"ArsiaAtGenesis": {
			genesisConfigFn: func(dc *genesis.DeployConfig) { dc.ActivateMantleForkAtGenesis(forks.MantleArsia) },
		},
		"ArsiaAfterGenesis": {
			genesisConfigFn: func(dc *genesis.DeployConfig) { dc.ActivateMantleForkAtOffset(forks.MantleArsia, 4) },
		},
	}
