...
// L2ProofEnv is a test harness for a fault provable L2 chain.
type L2ProofEnv struct {
	log       log.Logger
...
func NewL2ProofEnv[c any](t helpers.Testing, testCfg *TestCfg[c], tp *e2eutils.TestParams, batcherCfg *helpers.BatcherCfg, deployConfigOverrides ...DeployConfigOverride) *L2ProofEnv {
	...
	sd := e2eutils.SetupMantleNormal(t, dp, genesisAlloc)
...
func (env *L2ProofEnv) RunFaultProofProgramFromGenesis(t helpers.Testing, finalL2BlockNum uint64, checkResult CheckResult, fixtureInputParams ...FixtureInputParam) {
...
func (env *L2ProofEnv) RunFaultProofProgram(t helpers.Testing, l2ClaimBlockNum uint64, checkResult CheckResult, fixtureInputParams ...FixtureInputParam) {
	defaultParam := WithPreInteropDefaults(t, l2ClaimBlockNum, env.Sequencer.L2Verifier, env.Engine)
	combinedParams := []FixtureInputParam{defaultParam}
	combinedParams = append(combinedParams, fixtureInputParams...)
	RunFaultProofProgram(t, env.log, env.Miner, checkResult, combinedParams...)
...
func NewDeployParams(t helpers.Testing, tp *e2eutils.TestParams, params ...DeployParam) *e2eutils.DeployParams {
	dfault := e2eutils.MakeMantleDeployParams(t, tp)
	for _, apply := range params {
		apply(dfault)
	}
	return dfault
...
// batcher transaction.
func (env *L2ProofEnv) BatchAndMine(t helpers.Testing) {
	t.Helper()
