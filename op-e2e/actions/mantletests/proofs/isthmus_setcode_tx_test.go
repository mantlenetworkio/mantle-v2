package proofs

import (
	"bytes"
	"math/big"
	"math/rand"
	"testing"

	"github.com/ethereum/go-ethereum/common"
	"github.com/ethereum/go-ethereum/common/hexutil"
	"github.com/ethereum/go-ethereum/core/types"
	"github.com/ethereum/go-ethereum/core/vm/program"
	"github.com/holiman/uint256"
	"github.com/stretchr/testify/require"

	"github.com/ethereum-optimism/optimism/op-chain-ops/genesis"
	actionsHelpers "github.com/ethereum-optimism/optimism/op-e2e/actions/helpers"
	"github.com/ethereum-optimism/optimism/op-e2e/actions/mantletests/proofs/helpers"
	"github.com/ethereum-optimism/optimism/op-node/rollup/derive"
	"github.com/ethereum-optimism/optimism/op-service/testlog"
	"github.com/ethereum-optimism/optimism/op-service/testutils"
)

func Test_ProgramAction_SetCodeTx(gt *testing.T) {
	matrix := helpers.NewMatrix[any]()

	matrix.AddDefaultTestCases(
		nil,
		helpers.MantleArsiaOnly(),
		runSetCodeTxTypeTest,
	)

	matrix.Run(gt)
}

func runSetCodeTxTypeTest(gt *testing.T, testCfg *helpers.TestCfg[any]) {
	var (
		aa = common.HexToAddress("0x000000000000000000000000000000000000aaaa")
		bb = common.HexToAddress("0x000000000000000000000000000000000000bbbb")
	)

	t := actionsHelpers.NewDefaultTesting(gt)

	// hardcoded because it's not available until after we need it
	bobAddr := common.HexToAddress("0x14dC79964da2C08b23698B3D3cc7Ca32193d9955")

	// Create 2 contracts, (1) writes 42 to slot 42, (2) calls (1)
	store42Program := program.New().Sstore(0x42, 0x42)
	callBobProgram := program.New().Call(nil, bobAddr, 1, 0, 0, 0, 0)

	alloc := *actionsHelpers.DefaultAlloc
	alloc.L2Alloc = make(map[common.Address]types.Account)
	alloc.L2Alloc[aa] = types.Account{
		Code: store42Program.Bytes(),
	}
	alloc.L2Alloc[bb] = types.Account{
		Code: callBobProgram.Bytes(),
	}

	testCfg.Allocs = &alloc

	tp := helpers.NewTestParams()
	env := helpers.NewL2ProofEnv(t, testCfg, tp, helpers.NewBatcherCfg())
	require.Equal(gt, env.Bob.Address(), bobAddr)

	cl := env.Engine.EthClient()

	env.Sequencer.ActL2PipelineFull(t)
	env.Miner.ActEmptyBlock(t)
	env.Sequencer.ActL2StartBlock(t)

	aliceSecret := env.Alice.L2.Secret()
	bobSecret := env.Bob.L2.Secret()

	chainID := env.Sequencer.RollupCfg.L2ChainID

	// Sign authorization tuples.
	// The way the auths are combined, it becomes
	// 1. tx -> addr1 which is delegated to 0xaaaa
	// 2. addr1:0xaaaa calls into addr2:0xbbbb
	// 3. addr2:0xbbbb  writes to storage
	auth1, err := types.SignSetCode(aliceSecret, types.SetCodeAuthorization{
		ChainID: *uint256.MustFromBig(chainID),
		Address: bb,
		Nonce:   1,
	})
	require.NoError(gt, err, "failed to sign auth1")
	auth2, err := types.SignSetCode(bobSecret, types.SetCodeAuthorization{
		Address: aa,
		Nonce:   0,
	})
	require.NoError(gt, err, "failed to sign auth2")

	txdata := &types.SetCodeTx{
		ChainID:   uint256.MustFromBig(chainID),
		Nonce:     0,
		To:        env.Alice.Address(),
		Gas:       500000,
		GasFeeCap: uint256.NewInt(5000000000),
		GasTipCap: uint256.NewInt(2),
		AuthList:  []types.SetCodeAuthorization{auth1, auth2},
	}
	signer := types.NewIsthmusSigner(chainID)
	tx := types.MustSignNewTx(aliceSecret, signer, txdata)

	err = cl.SendTransaction(t.Ctx(), tx)
	require.NoError(gt, err, "failed to send set code tx")

	_, err = env.Engine.EngineApi.IncludeTx(tx, env.Alice.Address())
	require.NoError(t, err, "failed to include set code tx")

	env.Sequencer.ActL2EndBlock(t)

	// Verify delegation designations were deployed.
	bobCode, err := cl.PendingCodeAt(t.Ctx(), env.Bob.Address())
	require.NoError(gt, err, "failed to get bob code")
	want := types.AddressToDelegation(auth2.Address)
	if !bytes.Equal(bobCode, want) {
		t.Fatalf("addr1 code incorrect: got %s, want %s", common.Bytes2Hex(bobCode), common.Bytes2Hex(want))
	}
	aliceCode, err := cl.PendingCodeAt(t.Ctx(), env.Alice.Address())
	require.NoError(gt, err, "failed to get alice code")
	want = types.AddressToDelegation(auth1.Address)
	if !bytes.Equal(aliceCode, want) {
		t.Fatalf("addr2 code incorrect: got %s, want %s", common.Bytes2Hex(aliceCode), common.Bytes2Hex(want))
	}

	// Verify delegation executed the correct code.
	fortyTwo := common.BytesToHash([]byte{0x42})
	actual, err := cl.PendingStorageAt(t.Ctx(), env.Bob.Address(), fortyTwo)
	require.NoError(gt, err, "failed to get addr1 storage")

	if !bytes.Equal(actual, fortyTwo[:]) {
		t.Fatalf("addr2 storage wrong: expected %d, got %d", fortyTwo, actual)
	}

	// batch submit to L1. batcher should submit span batches.
	env.BatchAndMine(t)

	env.Sequencer.ActL1HeadSignal(t)
	env.Sequencer.ActL2PipelineFull(t)

}

// TestSetCodeTxSupportedBeforeArsia tests that SetCode transactions are already supported
// in Mantle Limb (before Arsia activation), because Mantle Skadi = Prague which introduced EIP-7702.
// This verifies that Arsia does NOT introduce SetCode support - it was already there.
func Test_ProgramAction_SetCodeTxSupportedBeforeArsia(gt *testing.T) {
	matrix := helpers.NewMatrix[any]()
	matrix.AddDefaultTestCases(
		nil,
		helpers.NewForkMatrix(helpers.MantleLimb), // Test in Limb (before Arsia)
		testSetCodeTxSupportedBeforeArsia,
	)
	matrix.Run(gt)
}

func testSetCodeTxSupportedBeforeArsia(gt *testing.T, testCfg *helpers.TestCfg[any]) {
	t := actionsHelpers.NewDefaultTesting(gt)
	tp := helpers.NewTestParams()

	// Setup: Configure fork activation times

	// - Limb activates at genesis (offset=0)
	// - Arsia activates 100 seconds after genesis (offset=100)
	env := helpers.NewL2ProofEnv(t, testCfg, tp, helpers.NewBatcherCfg(), func(dc *genesis.DeployConfig) {
		// Activate Limb at genesis
		limbOffset := hexutil.Uint64(0)
		dc.L2GenesisMantleLimbTimeOffset = &limbOffset

		// Activate Arsia 100 seconds after genesis
		// This allows us to test SetCode in both Limb and Arsia phases
		arsiaOffset := hexutil.Uint64(100)
		dc.L2GenesisMantleArsiaTimeOffset = &arsiaOffset
	})

	sequencer := env.Sequencer
	aliceSecret := env.Alice.L2.Secret()
	bobSecret := env.Bob.L2.Secret()
	chainID := env.Sequencer.RollupCfg.L2ChainID

	// Phase 1: Test SetCode in Limb (before Arsia)

	gt.Logf("========== Phase 1: Testing SetCode in Limb ==========")

	// Verify we're in Limb fork, not Arsia yet
	currentTime := sequencer.L2Unsafe().Time
	require.True(t, env.Sd.RollupCfg.IsMantleLimb(currentTime),
		"Should be in Limb fork")
	require.False(t, env.Sd.RollupCfg.IsMantleArsia(currentTime),
		"Should NOT be in Arsia fork yet")

	// Verify Skadi (Prague) is active, which enables SetCode
	require.True(t, env.Sd.RollupCfg.IsMantleSkadi(currentTime),
		"Skadi (Prague) should be active")

	gt.Logf("Current L2 time: %d, Genesis time: %d",
		currentTime, env.Sd.RollupCfg.Genesis.L2Time)

	// Build a block with SetCode transaction
	sequencer.ActL2StartBlock(t)

	// Create SetCode authorization: Bob authorizes Alice's address to use Bob's code
	auth1, err := types.SignSetCode(bobSecret, types.SetCodeAuthorization{
		ChainID: *uint256.MustFromBig(chainID),
		Address: env.Alice.Address(),
		Nonce:   0,
	})
	require.NoError(gt, err)

	// Create SetCode transaction sent by Alice
	txdata1 := &types.SetCodeTx{
		ChainID:   uint256.MustFromBig(chainID),
		Nonce:     0,
		To:        env.Bob.Address(),
		Gas:       100000,
		GasFeeCap: uint256.NewInt(1000000000),
		GasTipCap: uint256.NewInt(2),
		AuthList:  []types.SetCodeAuthorization{auth1},
	}

	signer := types.NewPragueSigner(chainID)
	tx1 := types.MustSignNewTx(aliceSecret, signer, txdata1)

	// Include the SetCode transaction in the block
	_, err = env.Engine.EngineApi.IncludeTx(tx1, env.Alice.Address())
	require.NoError(t, err, "SetCode tx should be accepted in Limb")

	sequencer.ActL2EndBlock(t)

	// Verify the block contains the SetCode transaction
	latestBlock := sequencer.L2Unsafe()
	block1 := env.Engine.L2Chain().GetBlockByHash(latestBlock.Hash)
	require.GreaterOrEqual(t, len(block1.Transactions()), 2,
		"Block should contain L1 info deposit + SetCode tx")

	gt.Logf(" Phase 1 Success: SetCode transaction included in Limb")
	gt.Logf("   Block number: %d, Tx count: %d",
		latestBlock.Number, len(block1.Transactions()))

	// Time Advancement: Move to Arsia activation

	gt.Logf("========== Advancing time to Arsia activation ==========")

	// Calculate how many blocks we need to advance
	// Arsia activates at genesis + 100 seconds
	// L2 BlockTime = 2 seconds per block
	genesisTime := env.Sd.RollupCfg.Genesis.L2Time
	arsiaActivationTime := genesisTime + 100
	currentTime = sequencer.L2Unsafe().Time

	gt.Logf("Genesis time: %d", genesisTime)
	gt.Logf("Current time: %d", currentTime)
	gt.Logf("Arsia activation time: %d", arsiaActivationTime)

	// Advance L1 and L2 blocks until Arsia activates
	// We need to be careful to advance both L1 and L2 in sync
	for currentTime < arsiaActivationTime {
		// Advance L1 block
		env.Miner.ActL1StartBlock(12)(t)
		env.Miner.ActL1EndBlock(t)

		// Advance L2 block
		sequencer.ActL1HeadSignal(t)
		sequencer.ActBuildToL1Head(t)

		// Update current time
		currentTime = sequencer.L2Unsafe().Time
	}

	gt.Logf("Advanced to time: %d", currentTime)

	// Verify Arsia is now active
	require.True(t, env.Sd.RollupCfg.IsMantleArsia(currentTime),
		"Arsia should be active now")
	require.True(t, env.Sd.RollupCfg.IsMantleLimb(currentTime),
		"Limb should still be active (Arsia includes Limb)")

	gt.Logf(" Arsia fork activated at time %d", currentTime)

	// Phase 2: Test SetCode in Arsia (after upgrade)

	gt.Logf("========== Phase 2: Testing SetCode in Arsia ==========")

	// Build another block with SetCode transaction
	sequencer.ActL2StartBlock(t)

	// Create a new SetCode authorization
	// This time: Alice authorizes Bob's address to use Alice's code
	// (reversed from Phase 1 to avoid nonce conflicts)
	auth2, err := types.SignSetCode(aliceSecret, types.SetCodeAuthorization{
		ChainID: *uint256.MustFromBig(chainID),
		Address: env.Bob.Address(),
		Nonce:   1,
	})
	require.NoError(gt, err)

	// Create SetCode transaction sent by Bob
	txdata2 := &types.SetCodeTx{
		ChainID:   uint256.MustFromBig(chainID),
		Nonce:     1,
		To:        env.Alice.Address(),
		Gas:       100000,
		GasFeeCap: uint256.NewInt(1000000000),
		GasTipCap: uint256.NewInt(2),
		AuthList:  []types.SetCodeAuthorization{auth2},
	}

	tx2 := types.MustSignNewTx(bobSecret, signer, txdata2)

	// Include the SetCode transaction in the block
	_, err = env.Engine.EngineApi.IncludeTx(tx2, env.Bob.Address())
	require.NoError(t, err, "SetCode tx should still be accepted in Arsia")

	sequencer.ActL2EndBlock(t)

	// Verify the block contains the SetCode transaction
	latestBlock = sequencer.L2Unsafe()
	block2 := env.Engine.L2Chain().GetBlockByHash(latestBlock.Hash)
	require.GreaterOrEqual(t, len(block2.Transactions()), 2,
		"Block should contain L1 info deposit + SetCode tx")

	gt.Logf(" Phase 2 Success: SetCode transaction still works in Arsia")
	gt.Logf("   Block number: %d, Tx count: %d",
		latestBlock.Number, len(block2.Transactions()))

	// Summary

	gt.Logf("========== Test Summary ==========")
	gt.Logf(" SetCode is supported in Limb (inherited from Skadi/Prague)")
	gt.Logf(" SetCode continues to work after Arsia upgrade")
	gt.Logf(" This proves SetCode support is NOT introduced by Arsia")
	gt.Logf(" Arsia only changes base fee calculation, not EVM opcodes")
}

func Test_ProgramAction_SetCodeTxWithContractCreationBitSet(gt *testing.T) {
	matrix := helpers.NewMatrix[any]()

	matrix.AddDefaultTestCases(
		nil,
		helpers.NewForkMatrix(helpers.MantleArsia),
		runSetCodeTxTypeWithContractCreationBitSetTest,
	)

	matrix.Run(gt)
}

func runSetCodeTxTypeWithContractCreationBitSetTest(gt *testing.T, testCfg *helpers.TestCfg[any]) {
	var (
		aa = common.HexToAddress("0x000000000000000000000000000000000000aaaa")
		bb = common.HexToAddress("0x000000000000000000000000000000000000bbbb")
	)

	t := actionsHelpers.NewDefaultTesting(gt)

	// hardcoded because it's not available until after we need it
	bobAddr := common.HexToAddress("0x14dC79964da2C08b23698B3D3cc7Ca32193d9955")

	// Create 2 contracts, (1) writes 42 to slot 42, (2) calls (1)
	store42Program := program.New().Sstore(0x42, 0x42)
	callBobProgram := program.New().Call(nil, bobAddr, 1, 0, 0, 0, 0)

	alloc := *actionsHelpers.DefaultAlloc
	alloc.L2Alloc = make(map[common.Address]types.Account)
	alloc.L2Alloc[aa] = types.Account{
		Code: store42Program.Bytes(),
	}
	alloc.L2Alloc[bb] = types.Account{
		Code: callBobProgram.Bytes(),
	}

	testCfg.Allocs = &alloc

	tp := helpers.NewTestParams()
	cfg := helpers.NewBatcherCfg()
	env := helpers.NewL2ProofEnv(t, testCfg, tp, cfg)
	sequencer := env.Sequencer
	miner := env.Miner
	batcher := env.Batcher
	chainID := env.Sequencer.RollupCfg.L2ChainID

	require.Equal(gt, env.Bob.Address(), bobAddr)
	u1 := sequencer.L2Unsafe()

	sequencer.ActL2EmptyBlock(t) // we'll inject the setcode tx in this block's batch

	signer := types.NewIsthmusSigner(chainID)

	rng := rand.New(rand.NewSource(0))
	tx := testutils.RandomSetCodeTx(rng, signer)

	txdata2 := &types.DynamicFeeTx{
		ChainID:   chainID,
		Nonce:     1,
		To:        nil,
		Gas:       500000,
		GasFeeCap: big.NewInt(5000000000),
		GasTipCap: big.NewInt(2),
	}
	tx2 := types.MustSignNewTx(env.Alice.L2.Secret(), signer, txdata2)

	batcher.ActL2BatchBuffer(
		t,
		actionsHelpers.WithBlockModifier(func(block *types.Block) *types.Block {
			// inject user tx into upgrade batch
			return block.WithBody(types.Body{Transactions: append(block.Transactions(), tx, tx2)})
		}),
		actionsHelpers.WithChannelModifier(
			derive.TestWithContractCreationBitsMod(func(ob *big.Int) *big.Int {
				// ensure contract bits are originally set to 0b10
				require.Equal(t, big.NewInt(0b10), ob, "expected contract creation bits to be 0b10")
				// flip bits
				return big.NewInt(0b01)
			})),
	)
	batcher.ActL2ChannelClose(t)

	batcher.ActL2BatchSubmit(t)

	miner.ActL1StartBlock(12)(t)
	miner.ActL1IncludeTxByHash(env.Batcher.LastSubmitted.Hash())(t)
	miner.ActL1EndBlock(t)

	sequencer.ActL1HeadSignal(t)
	sequencer.ActL2PipelineFull(t)

	l2safe := sequencer.L2Safe()
	s2block := env.Engine.L2Chain().GetBlockByHash(l2safe.Hash)
	require.Len(t, s2block.Transactions(), 0, "safe head should not contain either tx")
	require.Equal(t, u1, l2safe, "expected last block to be reorgd out due to setcode tx")

	// find a log with the error message as an attr
	recs := env.Logs.FindLogs(testlog.NewErrContainsFilter("to address is required for SetCodeTx"))
	require.GreaterOrEqual(t, len(recs), 1)
}
