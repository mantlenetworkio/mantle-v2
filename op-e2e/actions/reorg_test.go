...
func TestReorgOrphanBlock(gt *testing.T) {
	...
	require.NoError(t, miner.eth.TxPool().Add([]*types.Transaction{batchTx}, false)[0])
...
func TestReorgFlipFlop(gt *testing.T) {
	...
	miner.ActL1StartBlock(12)(t)
	require.NoError(t, miner.eth.TxPool().Add([]*types.Transaction{batchTxA}, false)[0])
	miner.ActL1IncludeTx(sd.RollupCfg.Genesis.SystemConfig.BatcherAddr)(t)
	miner.ActL1EndBlock(t)
	...
	miner.ActL1StartBlock(12)(t)
	require.NoError(t, miner.eth.TxPool().Add([]*types.Transaction{batchTxA}, false)[0]) // replay chain A batches, but now in A2 instead of A1
	miner.ActL1IncludeTx(sd.RollupCfg.Genesis.SystemConfig.BatcherAddr)(t)
	miner.ActL1EndBlock(t)
