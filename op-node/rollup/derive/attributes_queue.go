...
type AttributesBuilder interface {
	PreparePayloadAttributes(ctx context.Context, isSequencerMode bool, l2Parent eth.L2BlockRef, epoch eth.BlockID) (attrs *eth.PayloadAttributes, err error)
}
...
func (aq *AttributesQueue) createNextAttributes(ctx context.Context, batch *BatchData, l2SafeHead eth.L2BlockRef) (*eth.PayloadAttributes, error) {
	...
	defer cancel()
	attrs, err := aq.builder.PreparePayloadAttributes(fetchCtx, false, l2SafeHead, batch.Epoch())
	if err != nil {
		return nil, err
	}
