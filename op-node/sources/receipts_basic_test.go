			...
			if el.Method == "eth_getTransactionReceipt" {
				...
				if response[txHash] {
					...
					**(el.Result.(**types.Receipt)) = *recMap[txHash]
					...
					if err != nil {
						err = fmt.Errorf("%v; receipt[%d] error, hash %x", err, i, txHash)
					} else {
						err = fmt.Errorf("receipt[%d] error, hash %x", i, txHash)
					}
				}
			} else {
				if err != nil {
					err = fmt.Errorf("%v; unknown method %s", err, el.Method)
				} else {
					err = fmt.Errorf("unknown method %s", el.Method)
				}
