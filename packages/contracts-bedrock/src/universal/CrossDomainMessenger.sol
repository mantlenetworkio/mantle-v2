    ...
    CrossDomainMessengerLegacySpacer1
{
        ...
        virtual
    {
        ...
        _sendMessage(
            ...
            abi.encodeWithSelector(
                this.relayMessage.selector, messageNonce(), msg.sender, _target, 0, msg.value, _minGasLimit, _message
            )
        );
    ...
    /// @param _message                      Message to trigger the target address with.
    /// @param _minGasLimit                  Minimum gas limit that the message can be executed with.
    function sendMessage(address _target, bytes calldata _message, uint32 _minGasLimit) external payable virtual {
        // Triggers a message to the other messenger. Note that the amount of gas provided to the
        // message is the amount of gas requested by the user PLUS the base gas value. We want to
        // guarantee the property that the call to the target contract will always have at least
        // the minimum gas limit specified by the user.
        ...
        virtual
    {
        ...
        bytes32 versionedHash =
            Hashing.hashCrossDomainMessageV1(_nonce, _sender, _target, _mntValue, _value, _minGasLimit, _message);
